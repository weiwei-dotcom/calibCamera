...
bool cameraCalibrate::getIntrinsic(string cameraName) {
    //从文件中查找是否有同名的相机内参文件
    string IntrinsicDir("E:\\Visual_项目\\opencvSFM_VS\\calibData\\Intrinsic\\");// 内参文本文件路径
    string calibImgDir("E:\\Visual_项目\\opencvSFM_VS\\calibData\\Img\\");       // 标定图像文件路径
    string IntrinsicFile = IntrinsicDir + cameraName + ".txt";                   // 内参文本文件
    string calibImgFile = calibImgDir + cameraName + "*";                        // 标定图像文件
    // 读取相机的内参文件
